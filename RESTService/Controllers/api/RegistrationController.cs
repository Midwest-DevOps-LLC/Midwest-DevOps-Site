            ...
            try
            {
                using (var emailBLL = new RESTBLL.EmailRegistrations(MDO.Utility.Standard.ConnectionHandler.GetConnectionString()))
                {
                    ...
                    if (emailID != null)
                    {
                        ...
                        return MDO.EmailHandler.Standard.EmailHandler.SendEmail(MDO.EmailHandler.Standard.EmailHandler.EmailAddresses.noreply, toAddress.Address, "mpbhhkpodykjutef", $"Verify Email - {siteByApplication.Name}", emailText, true);
                    }
                }
            }
            catch (Exception ex)
